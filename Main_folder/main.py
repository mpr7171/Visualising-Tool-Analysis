...
def combined_analytics():
    ...
    endsem_df = pd.DataFrame({'End Sem': np.array(end_sem_grades)})
    combined = pd.DataFrame({'Combined': np.array(result)})
    ...
    fig.add_trace(go.Box(y=minor2_df['Minor2'], name="Minor 2"))
    fig.add_trace(go.Box(y=endsem_df['End Sem'], name="End Semester"))
    fig.add_trace(go.Box(y=combined['Combined'], name="Total"))
    fig.update_layout(showlegend=True)
    ...
    std = np.std(result)
    x = np.linspace(result.min(), result.max(), 100)
    ...
    colors = ['red', 'blue', 'green']  # Specify colors for each percentile line
    percentiles = [75, 90, 95]
    percentile_values = np.percentile(result, percentiles)
    ...
    fig3 = go.Figure(data=[go.Histogram(x=result)])


    fig3.update_layout(
        title="Histogram of the m1, m2 and end sem Grades",
        xaxis_title="Marks",
        yaxis_title="Frequency"
    )
